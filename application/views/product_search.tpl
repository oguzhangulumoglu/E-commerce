                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select name="category" class="form-control" style="display: initial;float: none; height: 45px; position: initial; border: 0; border-radius: 0;color: #a3a3a3">
                                        ...
                                        {foreach from=$category name=foo2 item=brands}
                                            <option {if $smarty.get.category eq $brands->id}selected="selected"{/if} value="{$brands->id}">{$brands->name}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                <div class="col-md-2 row" style="margin-right:0!important">
                                    ...
                                    <select name="tire_width" class="form-control" style="display: initial;float: none; height: 45px; position: initial; border: 0; border-radius: 0;color: #a3a3a3">
                                        ...
                                        {foreach from=$select_tire_width name=foo2 item=tire_width}
                                            <option {if $smarty.get.tire_width eq $tire_width->id}selected="selected"{/if} value="{$tire_width->id}">{$tire_width->name}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                <div class="col-md-2 row" style="margin-right:0!important">
                                    ...
                                    <select name="tire_rate" class="form-control" style="display: initial;float: none; height: 45px; position: initial; border: 0; border-radius: 0;color: #a3a3a3">
                                        ...
                                        {foreach from=$select_tire_rate name=foo2 item=tire_rate}
                                            <option {if $smarty.get.tire_rate eq $tire_rate->id}selected="selected"{/if} value="{$tire_rate->id}">{$tire_rate->name}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                <div class="col-md-2 row" style="margin-right:0!important">
                                    ...
                                    <select name="rim_diameter" class="form-control" style="display: initial;float: none; height: 45px; position: initial; border: 0; border-radius: 0;color: #a3a3a3">
                                        ...
                                        {foreach from=$select_rim_diameter name=foo2 item=rim_diameter}
                                            <option {if $smarty.get.rim_diameter eq $rim_diameter->id}selected="selected"{/if} value="{$rim_diameter->id}">{$rim_diameter->name}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                <div class="col-md-2 row" style="margin-right:0!important">
                                    ...
                                    <select name="season" class="form-control" style="display: initial;float: none; height: 45px; position: initial; border: 0; border-radius: 0;color: #a3a3a3">
                                        ...
                                        {foreach from=$select_season name=foo2 item=season}
                                            <option {if $smarty.get.season eq $season->id}selected="selected"{/if} value="{$season->id}">{$season->name}</option>
                                        {/foreach}
                                    </select>
                                </div>
                                <div class="col-md-3 row" style="margin-right:0!important">
                                    <label for="">Ürün adı veya Kodu</label>
                                    <input type="text" name="search" class="form-control" {if $smarty.get.search}value="{$smarty.get.search}"{/if} placeholder="Ürün ara">
                        ...
                        <div class="search-results">
                                                    ...
                                                    {if $status == "normal"}
                                                        <form action="{base_url("/main/product_update")}" method="post">
                                                            ...
                                                            <div class="form-group">
                                                                <label for="">Yıl:</label>
                                                                <input type="text" name="year" value="{$product->year}" class="form-control" placeholder="2011" required="">
                                                            </div>
                                                            <div class="form-group">
                            ...
                            <ul class="pagination">
                                {if $page > 1}
                                    <li><a href="{base_url("main/product_search")}/1{if $exp[1]}?{$exp[1]}{/if}"> << </a></li>
                                    <li><a href="{base_url("main/product_search")}/{if $complete <= 1}1{else}{$complete-1}{/if}{if $exp[1]}?{$exp[1]}{/if}"> < </a></li>
                                {/if}
                                <li class="active"><a href="{base_url("main/product_search")}/{$complete}{if $exp[1]}?{$exp[1]}{/if}"> {$complete} </a></li>
                                {if $page > 1}
                                    <li><a href="{base_url("main/product_search")}/{if $page <= $complete+1}{$page}{else}{$complete+1}{/if}{if $exp[1]}?{$exp[1]}{/if}"> > </a></li>
                                    <li><a href="{base_url("main/product_search")}/{$page}{if $exp[1]}?{$exp[1]}{/if}"> >> </a></li>
                                {/if}
                            </ul>

                            <ul class="pagination hidden">
                                {for $foo=1 to $page}
                                    <li{if $foo eq $complete} class="active"{/if}><a href="{base_url("main/product_search/")}/{$foo}">{$foo}</a></li>
                                {/for}
                            </ul>
                        </div>
                    </div>
